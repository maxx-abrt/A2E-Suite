...
Installer Bilan crée un dossier **Bilan** dans la navigation, avec :
...
L'installation prépare aussi, sans intervention :
...
Les lignes machine sont en lecture seule sauf leur commentaire, et rejouer un
événement ne crée jamais de doublon. Une source supprimée ne fait pas
disparaître la ligne : elle est retirée, pour que l'export d'un exercice clos
garde sa piste.
...
```bash
export PATH=/app/.runtime/node24/bin:$PATH

# Construire et valider le manifeste (objets, vues, navigation, fonctions)
node /app/node_modules/.bin/twenty dev:build .
