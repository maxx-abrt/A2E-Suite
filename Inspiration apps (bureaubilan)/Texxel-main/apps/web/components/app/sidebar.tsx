  ...
  return (
    <>
      ...
      <aside
        ...
        className={cn(
          "fixed inset-y-0 left-0 z-50 flex w-[min(320px,90vw)] flex-col border-r border-sidebar-border bg-sidebar text-sidebar-foreground md:static md:w-[var(--sb-w)] md:translate-x-0",
      ...
      >
        ...
        <div className="flex items-center gap-1 p-3">
          <DropdownMenu>
            ...
            <DropdownMenuContent align="start" className="w-64">
              ...
              <DropdownMenuSeparator />
              <DropdownMenuItem data-testid="sidebar-create-workspace" onClick={() => router.push("/app/settings?new=1")} className="gap-2 text-primary">
          ...
          <button className="md:hidden" aria-label={tx("close")} data-testid="sidebar-mobile-close" onClick={onClose}><CloseCircle variant="Bulk" size={20} /></button>
        </div>

        {/* Search */}
        <div className="px-3">
          <button data-testid="sidebar-search" onClick={onSearch} className="flex w-full items-center gap-2 rounded-xl border border-sidebar-border bg-background px-3 py-2 text-sm text-muted-foreground hover:bg-sidebar-accent">
            <SearchNormal1 variant="Bulk" size={18} /> {t("search")}
            <span className="ml-auto rounded-md border border-border px-1.5 text-xs">⌘K</span>
          </button>
        </div>

        <div className="mx-3 mt-3 flex items-center rounded-lg border border-sidebar-border bg-sidebar p-0.5" role="group" aria-label={tx("navigationMode")}>
        ...
        >
          {!treeOnly && favorites && favorites.length > 0 && (
            <div className="mb-3">
              <button
          ...
          <div
            ref={setRootRef}
            className={cn(
              "sticky top-0 z-10 flex items-center justify-between rounded-lg bg-sidebar px-2 py-2 transition-colors",
          ...
          {sectionOpen("private") && (
            <div
              ...
              className={cn(
                ...
                activeDrag && "outline outline-1 outline-dashed outline-primary/30",
                isOverRootArea && activeDrag && "bg-primary/[0.07] ring-2 ring-primary/40 border-primary/50",
              )}
            >
              {docs === undefined && <div role="status" aria-label={tx("loadingPages")} data-testid="tree-loading" className="space-y-3 p-3">{[0,1,2,3].map(i => <div key={i} className="h-5 animate-pulse rounded bg-muted" style={{ width: `${85-i*12}%` }} />)}</div>}
              <DocumentTree
                docs={deferredDocs ?? []}
              ...
              />
              {docs && docs.length === 0 && (
                <button data-testid="tree-empty-create" onClick={() => onCreate()} className="mt-1 flex w-full items-center gap-2 rounded-lg px-3 py-1.5 text-sm text-muted-foreground hover:bg-sidebar-accent">
        ...
        <div className="space-y-0.5 border-t border-sidebar-border p-3">
          ...
          <Link href="/app/settings" data-testid="sidebar-settings-link" onClick={onClose} className={cn("flex items-center gap-2.5 rounded-xl px-3 py-2 text-sm hover:bg-sidebar-accent", isActive("/app/settings") && "bg-sidebar-accent font-medium")}>
...
function DraggableFavorite({ doc, onNavigate, active }: { doc: any; onNavigate: () => void; active?: boolean }) {
  ...
  return (
    <div
      ...
      style={style}
      className={cn(
        "relative flex items-center gap-2 rounded-md px-3 py-1.5 text-[13px] hover:bg-sidebar-accent",
