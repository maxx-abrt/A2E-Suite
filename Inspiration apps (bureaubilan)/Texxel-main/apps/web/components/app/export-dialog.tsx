...
export function ExportDialog({
  ...
  const [format, setFormat] = useState<"pdf" | "docx" | "odt">("pdf");
  ...
  const compatibility = useMemo(() => {
    if (format === "odt") return tx("odtFont");
  ...
  const blocks = () => includeTitle
    ? [{
        ...
        children: [],
      }, ...exportableBlocks(editor.document)]
    : exportableBlocks(editor.document);
  ...
  const doPdf = async () => {
    ...
    const [math, diagram] = await Promise.all([import("@blocknote/math-block/pdf-exporter"), import("@blocknote/diagram-block/pdf-exporter")]);
    const mappings: any = {
  ...
  const doDocx = async () => {
    ...
    const [math, diagram] = await Promise.all([import("@blocknote/math-block/docx-exporter"), import("@blocknote/diagram-block/docx-exporter")]);
    const mappings: any = {
  ...
  const exportNow = async () => {
    ...
    try {
      ...
      setProgress(20);
      const blob = format === "pdf" ? await doPdf() : format === "docx" ? await doDocx() : await doOdt();
        ...
        <div className="space-y-5 p-5">
          <section>
            ...
            <div className="mt-2 grid grid-cols-3 gap-2">
              {(["pdf", "docx", "odt"] as const).map((item) => (
                <button key={item} type="button" disabled={busy} aria-pressed={format === item} onClick={() => setFormat(item)} className={cn("rounded-xl border p-3 text-left", format === item ? "border-primary bg-[var(--flux-coral-soft)]" : "border-border bg-card hover:bg-muted/40")} data-testid={`export-format-${item}`}>
                  <span className="flex items-center gap-2 text-sm font-semibold"><DocumentText variant="Bulk" size={17} /> {item.toUpperCase()}</span>
                  <span className="mt-1 block text-xs text-muted-foreground">{item === "odt" ? tx("odtDescription") : t(item === "pdf" ? "pdfDescription" : "docxDescription")}</span>
                </button>
              ))}
            </div>
          </section>
          <section className="rounded-xl border border-border bg-card p-4">
            ...
            <div className="grid gap-3 sm:grid-cols-2">
              <label className="text-xs text-muted-foreground">{t("pageSize")}<select disabled={busy || format === "odt"} value={pageSize} onChange={(event) => setPageSize(event.target.value)} className="mt-1 h-9 w-full rounded-lg border border-border bg-background px-2 text-sm text-foreground outline-none focus:ring-2 focus:ring-ring" data-testid="export-page-size"><option value="A4">A4</option><option value="LETTER">{t("letter")}</option><option value="LEGAL">{t("legal")}</option></select></label>
              <label className="text-xs text-muted-foreground">{t("margins", { margin })}<input disabled={busy || format === "odt"} type="range" min="24" max="72" step="4" value={margin} onChange={(event) => setMargin(Number(event.target.value))} className="mt-2 w-full accent-[var(--primary)]" data-testid="export-margin" /></label>
            </div>
            {format === "odt" && <p className="mt-2 text-xs text-muted-foreground">{tx("odtLayout")}</p>}
