"use client";
import { serializeEditor } from "@/lib/editor-review";
...
export function DocumentView({ documentId }: { documentId: Id<"flux_documents"> }) {
  ...
  const saveQueue = useDocumentSave(documentId);
  ...
  return (
    <div className={cn("min-h-full pb-32", focusMode && "fixed inset-0 z-[70] overflow-y-auto bg-background pb-24")} data-testid="document-view" data-focus-mode={focusMode}>
      ...
      <div className={cn("sticky top-0 z-40 flex items-center gap-1 border-b border-border bg-background/85 px-3 py-2 backdrop-blur md:px-6", focusMode && "mx-auto max-w-[1180px] rounded-b-2xl border-x bg-card/95 shadow-sm")}>
        ...
        <span className="ml-2 flex shrink-0 items-center gap-1 text-xs text-muted-foreground" data-testid="doc-save-state" role="status" aria-live="polite">
          {saveQueue.status === "error" ? <button type="button" data-testid="doc-save-retry" onClick={saveQueue.retry} className="text-destructive underline underline-offset-2">{tx("savedError")} · {tx("retry")}</button> : saving ? (
        ...
        <div className="ml-auto flex items-center gap-1.5">
          ...
          <button
            ...
            onClick={openExport}
            className="hidden h-8 items-center gap-1.5 rounded-lg border border-border px-2.5 text-xs font-semibold text-foreground hover:bg-muted sm:flex"
            data-testid="doc-export-open" aria-label={tDocs("export.open")}
          >
            <DocumentDownload variant="Bulk" size={15} /><span className="hidden lg:inline">{tDocs("export.open")}</span>
          </button>
          <ShareMenu
            doc={doc}
          ...
          <DropdownMenu>
            ...
            <DropdownMenuContent align="end" className="w-60">
              ...
              <DropdownMenuSeparator />
              <DropdownMenuItem data-testid="doc-cover-change" onClick={() => fileRef.current?.click()} className="gap-2">
                ...
                <div className="flex gap-2 pt-1">
                  ...
                  <button
                    ...
                    onClick={async () => {
                      ...
                      try {
                        const ed = editorRef.current;
                        const plainContent = ed ? serializeEditor(ed) : doc.content ?? "";
                        const { ciphertext, salt, iv } = await encryptContent(plainContent, passphraseInput);
                        await setLock({ documentId, content: ciphertext, isLocked: true, passphraseSalt: salt, lockIv: iv, passphraseHint: passphraseHintInput.trim() || undefined });
