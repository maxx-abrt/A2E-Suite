...
export class ConvexThreadStore extends ThreadStore {
  ...
  constructor(
    ...
    private role: "comment" | "editor",
  ) {
    super(new DefaultThreadStoreAuth(userId, role));
  }

  /**
   * Late-bind user id / role. The store (and therefore the editor) is created
   * once per document; `userId` may resolve after mount. The base auth object
   * has no setters, so we swap in a rebuilt one.
   */
  bindIdentity(userId: string, role: "comment" | "editor") {
    if (this.userId === userId && this.role === role) return;
    this.userId = userId;
    this.role = role;
    (this as any).auth = new DefaultThreadStoreAuth(userId, role);
  }
  ...
  updateFromServer(rows: ServerThread[] | undefined) {
    if (!rows) return;
    // Skip when nothing actually changed: this runs on every Convex re-emission
    // (anchor patches, presence…). Emitting a fresh Map each time re-renders the
    // CommentsExtension and re-triggers anchor rehydration — a freeze loop.
