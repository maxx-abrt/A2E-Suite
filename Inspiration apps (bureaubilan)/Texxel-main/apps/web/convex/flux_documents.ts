import { planTreeMove } from "../lib/document-tree-model";
 ...
 */
const compareDocs = compareSortKeys;
...
export const create = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    await validateParent(ctx, args.parentId, args.workspaceId, userId);
    const now = Date.now();
...
async function validateParent(ctx: MutationCtx, parentId: Id<"flux_documents"> | undefined, workspaceId: Id<"workspaces">, userId: Id<"users">, sourceId?: Id<"flux_documents">) {
  const seen = new Set<string>();
  let cursor = parentId;
  while (cursor) {
...
export const move = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    await validateParent(ctx, args.parentId, doc.workspaceId, userId, args.documentId);
