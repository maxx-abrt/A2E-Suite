...
| M1 | **One-command app provisioning** (kills G1) | Deploying the suite = all A2E apps registered + catalog-visible; Settings → Applications shows the A2E section on a fresh server | (a) Image carries built app manifests: Dockerfile gains `twenty-apps` build stages (SDK `dev:build` per app in CI) + artifacts, and the server registers them on boot (reuse the existing application-sync/registration path — no invented platform); (b) `docker compose up` on a clean volume yields 6 registered apps without manual CLI, verified in the A2E section; (c) preset install works on that same deployment; (d) uninstall-preflight still refuses populated apps; (e) upgrade path 0.x → next on a populated workspace; (f) CI workflow (cd-docker-image) builds apps into the GHCR image so pulls are complete | partial — 2026-09-22 orchestrator: all 5 installable apps (documents/drive/bilan/projects/chat) INSTALLED live on the dev server incl. the US-074 projects fix re-proof (legs b-install/e-install level); docker absent on this machine, image-baking (a)/(f) and clean-volume compose (b) unrunnable. — 2026-09-23 orchestrator (phase-01-report): non-Docker provisioning legs verified live on a fresh DB — `database:init:prod` ran the new 2.39.0 `AddBundledAppSourcePath` instance command (column added, cursor row completed); `app:provision-bundled` dry-ran all 5 tarballs and a real run registered A2E Drive `BUNDLED`+`isPreInstalled` with its FS path (idempotent re-run skips); `install-pre-installed-apps` installed Drive on `YCombinator` from the bundled path and skipped `Apple` (already installed). Two HEAD-blocking defects fixed by the orchestrator: the instance command was registered twice (module + `INSTANCE_COMMANDS`) and crashed `upgrade` on boot; `BUNDLED` was unhandled in the exportability guard (typecheck red). Docker still absent → (a)/(f)/(b) remain unrunnable. Open finding for M1(e): on an instance whose cursor already sits in the 2.39.0 workspace segment, cursor-based `upgrade` resumes at the segment start and never visits the new instance command (index 319 < 320); fresh installs via `database:init:prod` are unaffected — 2026-09-24 20:25 CEST orchestrator: **FIXED by US-080** (resume at the first unattempted instance command preceding the segment; no workspace-scoped cursor rows for instance steps; unit 3/3 + `inserted-instance-command` integration 4/4 green on my run) and **install-log hygiene by US-081** (SAME_VERSION throws `APP_ALREADY_INSTALLED` before the state flip/rollback, no boot `ERROR`/failed-upgrade metric; `failing-application-version-progression` integration 3/3 + server `tsgo` clean). Docker legs (a)/(f)/(b) still unrunnable (no docker). |
| M2 | **Presets that build a workspace** (kills G2/G4/G5) | Persona choice → apps installed AND starter content seeded → land on a useful Home with one primary action | (a) All 6 apps added to presets where personas need them (decision D-B1 executed: Bureau = documents+projects bundle entry); (b) live seeding verified rows > 0 for each persona (fix the post-install hook defect); (c) preview shows apps + content + blocked items honestly; (d) retry idempotent — no duplicate seeds; (e) individual→CRM→individual nav restoration e2e green; (f) onboarding copy reflects the real product (Bureau/Bilan wording) | partial — P1.6b/c unit-verified; live seeding + persona expansion open. — 2026-09-23 orchestrator (phase-01-report): the US-076 INSTALL_APP idempotency defect blocking honest retry completion for (c)/(d) is fixed and verified live — STUDENT apply on `Apple` with A2E Documents 0.2.0 already installed returns INSTALL_APP `SUCCEEDED` (was `FAILED`/`APP_ALREADY_INSTALLED`) and `appliedTemplateKeyVersion` set; the follow-up CRM apply restored the 3 managed nav rows (US-075 provenance intact) |
...
- [~] **P1.6c Front onboarding/Settings (after P1.6b):** preview apps,
      ...
      holds at every step. Remaining open: authenticated codegen and the
      Settings browser journey.
      — 2026-09-23 orchestrator: the INSTALL_APP same-version defect is
      CLOSED (US-076 + live re-proof) — STUDENT apply on `Apple` with A2E
      Documents 0.2.0 already installed → INSTALL_APP `SUCCEEDED`,
      NAVIGATION_VISIBILITY/SET_WORKSPACE_TEMPLATE `SUCCEEDED`,
      `appliedTemplateKeyVersion` STUDENT v1 (exact scenario that failed in
      the 20:15 pass); the CRM apply that followed restored b001/b004/b005
      (provenance) and reset the template. Onboarding suite 8 suites / 83
      tests green on my re-run.
...
- [x] **P4C.3 Recurrence (after P4C.2):** daily/weekly/monthly, interval,
      ...
      — 2026-09-24 20:25 CEST orchestrator: TICKED (phase-04-report +
      `tasks/live-verify/p4c-verify.mjs` 12/12 on the live /calendar):
      view-level occurrence expansion ships (US-082) — a weekly Mon+Wed
      "ends after 6" series created through the composer renders 4/4 in
      September, spills 2 into the October grid, is bounded to 0 by
      November and is reload-stable; the rule editor (US-083) round-trips
      a whole-series edit/reopen exactly (repeat=weekly, ends=count,
      count=6, Mon+Wed pressed), and the this-vs-series scope dialog
      appears on the day edit surface. Repo run (not the executors' scratch
      harness): calendar jest 22 suites / 229 green, `tsgo` clean.
...
- [~] **P4C.5 Task/project links (after P4.1/P4C.2):** overlay task due dates,
      ...
      — 2026-09-24 20:25 CEST orchestrator (phase-04-report): first two legs
      verified live on /calendar — the read-only deadline overlay (US-084)
      renders standard-`task` chips, opens the task in the side panel and
      issues no `dueAt` query while hidden; day/week quick-create (US-085)
      writes the standard task. Remaining legs (opt-in event↔task link,
      "open project/task from event", confirm before moving linked
      deadlines) need the source-of-truth decision + a relation on standard
      `calendarEvent` (deferred).
