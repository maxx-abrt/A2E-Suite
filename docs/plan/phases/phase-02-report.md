...
## 2026-09-26 19:10 CEST — orchestrator — UNREPORTED auto-commit a2a4932f (P2.6 dock removal + re-home) — removal verified, re-home PARTIAL, build repaired

**Attribution:** `a2a4932f` ("Auto-generated changes", emergent-agent-e1, 2026-09-26T16:16:33Z) landed the whole `docs/tasks/p2.6-shell-cleanup-and-readiness.md` queue in one commit with **no executor report** — its own `.emergent/emergent_todos.json` still lists "Append executor reports to phase files" as `in_progress`. It spans P2.6 (this phase), P4.1/P4.2/P4C.4 (phase-04), P3.2/P3.3 (phase-03). Diff audited: only the approved removal + additive fields; no i18n catalogs, no secrets, no other deletion.
**Removal half verified:** `WorkbenchWidgetDock` unmounted from `MainAppLayoutWithSidePanel.tsx`; `modules/workbench-dock/**`, the three `register*Widget*` side-effect modules/tests and `RootStackingContextZIndices.WorkbenchWidgetDock` deleted. `packages/twenty-front` `npx tsgo -p tsconfig.json --noEmit` → exit 0; `npx jest src/modules/home-dashboard src/modules/first-open-help src/modules/drive` → 38 suites / 182 tests; `npx jest --findRelatedTests` on MobileHomePage/MainAppLayoutWithSidePanel/SidePanelSearchRecordsPage/useHelpSearchResultItems → 4 suites / 48 tests. Desktop `AppPath.Home` now renders the widget grid reusing the self-fetching `*Widget` wrappers. No dangling import of the deleted modules.
**Re-home half PARTIAL — PLAN P2.6 stays `[~]` (not ticked):** the acceptance is "every re-homed feature is reachable from a native surface", and two are orphaned. (1) **Help:** `FirstOpenHelpWidget`/`FirstOpenHelpPanel` now have no renderer; `useHelpSearchResultItems` adds a Cmd+K row that only navigates to `AppPath.Home`, but the new Home grid renders suggestions/tasks/events/activity/focus/contributions — not help. The hook's own comment ("remain accessible on the /home route itself") is false. (2) **Drive usage:** `DriveUsageWidget` is imported by nothing and `pages/drive/DrivePage.tsx` has no usage element — the brief's "Drive usage → the Drive surface" was not done.
**Do not redo:** the dock deletion, the Home widget grid, the `useHelpSearchResultItems` command row and the 182+48 green suites — all correct and additive. Never re-create `modules/workbench-dock` or a right rail (D-Shell).
**Next:** executor — render `FirstOpenHelpWidget` on the `AppPath.Home` surface (or open its panel from the Cmd+K row) and mount `DriveUsageWidget` on the Drive page; then orchestrator Tier-2 browser proof that no rail remains at desktop/tablet/mobile.
